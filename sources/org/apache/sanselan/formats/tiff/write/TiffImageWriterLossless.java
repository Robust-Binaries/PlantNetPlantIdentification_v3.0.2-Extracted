    ...
    private void dumpElements(ByteSource byteSource, List list) throws IOException {
        ...
        for (int i2 = 0; i2 < list.size(); i2++) {
            ...
            if (tiffElement.offset > i) {
                int i3 = tiffElement.offset - i;
                StringBuffer stringBuffer = new StringBuffer();
                stringBuffer.append("gap of ");
                stringBuffer.append(i3);
                stringBuffer.append(" bytes.");
                Debug.debug(stringBuffer.toString());
                byte[] block = byteSource.getBlock(i, i3);
                if (block.length > 64) {
            ...
            stringBuffer2.append(tiffElement.getElementDescription());
            stringBuffer2.append(" (");
            ...
            if (tiffElement instanceof TiffDirectory) {
                TiffDirectory tiffDirectory = (TiffDirectory) tiffElement;
                StringBuffer stringBuffer3 = new StringBuffer();
                stringBuffer3.append("\tnext Directory Offset: ");
                stringBuffer3.append(tiffDirectory.nextDirectoryOffset);
        ...
        try {
            ...
            for (int i2 = 0; i2 < arrayList2.size(); i2++) {
                ...
                if (tiffImageData != null) {
                    DataElement[] imageData = tiffImageData.getImageData();
                    for (DataElement add : imageData) {
                        arrayList.add(add);
                    }
                }
            }
