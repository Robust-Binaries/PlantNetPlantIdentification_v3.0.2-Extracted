    ...
    private static boolean isLocationOnEdgeOrPath(LatLng latLng, List<LatLng> list, boolean z, boolean z2, double d) {
        ...
        if (z2) {
            ...
            while (it.hasNext()) {
                ...
                double d7 = d6;
                double radians7 = Math.toRadians(latLng5.latitude);
                double mercator3 = MathUtil.mercator(radians7);
                Iterator it2 = it;
                double radians8 = Math.toRadians(latLng5.longitude);
    ...
    public static List<LatLng> simplify(List<LatLng> list, double d) {
        ...
        if (size >= 1) {
            ...
            if (d > 0.0d) {
                ...
                LatLng latLng = null;
                if (isClosedPolygon) {
                    latLng = (LatLng) list2.get(list.size() - 1);
                ...
                if (size > 2) {
                    ...
                    while (stack.size() > 0) {
                        ...
                        for (int i4 = iArr[0] + 1; i4 < iArr[1]; i4++) {
                            double distanceToLine = distanceToLine((LatLng) list2.get(i4), (LatLng) list2.get(iArr[0]), (LatLng) list2.get(iArr[1]));
