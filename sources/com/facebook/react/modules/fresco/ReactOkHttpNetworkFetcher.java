...
class ReactOkHttpNetworkFetcher extends OkHttpNetworkFetcher {
    ...
    public void fetch(OkHttpNetworkFetchState okHttpNetworkFetchState, Callback callback) {
        ...
        fetchWithRequest(okHttpNetworkFetchState, callback, new Builder().cacheControl(new CacheControl.Builder().noStore().build()).url(uri.toString()).headers(Headers.m189of(headers)).get().build());
    }
}
