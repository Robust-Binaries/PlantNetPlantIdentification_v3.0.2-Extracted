...
class RedBoxDialog extends Dialog implements OnItemClickListener {
    ...
    private static class CopyToHostClipBoardTask extends AsyncTask<String, Void, Void> {
        ...
        public Void doInBackground(String... strArr) {
            try {
                ...
                FLog.m49e(ReactConstants.TAG, "Could not copy to the host clipboard", (Throwable) e);
            }
            return null;
        }
    }
    ...
    private static class OpenStackFrameTask extends AsyncTask<StackFrame, Void, Void> {
        ...
        public Void doInBackground(StackFrame... stackFrameArr) {
            try {
                ...
                FLog.m49e(ReactConstants.TAG, "Could not open stack frame", (Throwable) e);
            }
            return null;
        }

        private static JSONObject stackFrameToJson(StackFrame stackFrame) {
            return new JSONObject(MapBuilder.m124of(UriUtil.LOCAL_FILE_SCHEME, stackFrame.getFile(), "methodName", stackFrame.getMethod(), StackTraceHelper.LINE_NUMBER_KEY, Integer.valueOf(stackFrame.getLine()), StackTraceHelper.COLUMN_KEY, Integer.valueOf(stackFrame.getColumn())));
        }
    }
    ...
    private static class StackAdapter extends BaseAdapter {
        ...
        private static class FrameViewHolder {
            ...
            private FrameViewHolder(View view) {
                this.mMethodView = (TextView) view.findViewById(C0788R.C0791id.rn_frame_method);
                this.mFileView = (TextView) view.findViewById(C0788R.C0791id.rn_frame_file);
            }
        }
        ...
        public View getView(int i, View view, ViewGroup viewGroup) {
            ...
            if (i == 0) {
                if (view != null) {
                    ...
                    textView = (TextView) LayoutInflater.from(viewGroup.getContext()).inflate(C0788R.layout.redbox_item_title, viewGroup, false);
                }
                textView.setText(this.mTitle);
                return textView;
            }
            if (view == null) {
                view = LayoutInflater.from(viewGroup.getContext()).inflate(C0788R.layout.redbox_item_frame, viewGroup, false);
    ...
    protected RedBoxDialog(Context context, DevSupportManager devSupportManager, @Nullable RedBoxHandler redBoxHandler) {
        super(context, C0788R.style.Theme_Catalyst_RedBox);
        requestWindowFeature(1);
        setContentView(C0788R.layout.redbox_view);
        this.mDevSupportManager = devSupportManager;
        this.mDoubleTapReloadRecognizer = new DoubleTapReloadRecognizer();
        this.mRedBoxHandler = redBoxHandler;
        this.mStackView = (ListView) findViewById(C0788R.C0791id.rn_redbox_stack);
        this.mStackView.setOnItemClickListener(this);
        this.mReloadJsButton = (Button) findViewById(C0788R.C0791id.rn_redbox_reload_button);
        this.mReloadJsButton.setOnClickListener(new OnClickListener() {
            public void onClick(View view) {
                RedBoxDialog.this.mDevSupportManager.handleReloadJS();
            }
        });
        this.mDismissButton = (Button) findViewById(C0788R.C0791id.rn_redbox_dismiss_button);
        this.mDismissButton.setOnClickListener(new OnClickListener() {
            public void onClick(View view) {
                RedBoxDialog.this.dismiss();
            }
        });
        this.mCopyToClipboardButton = (Button) findViewById(C0788R.C0791id.rn_redbox_copy_button);
        ...
        if (redBoxHandler2 != null && redBoxHandler2.isReportEnabled()) {
            this.mLoadingIndicator = (ProgressBar) findViewById(C0788R.C0791id.rn_redbox_loading_indicator);
            this.mLineSeparator = findViewById(C0788R.C0791id.rn_redbox_line_separator);
            this.mReportTextView = (TextView) findViewById(C0788R.C0791id.rn_redbox_report_label);
            this.mReportTextView.setMovementMethod(LinkMovementMethod.getInstance());
            this.mReportTextView.setHighlightColor(0);
            this.mReportButton = (Button) findViewById(C0788R.C0791id.rn_redbox_report_button);
