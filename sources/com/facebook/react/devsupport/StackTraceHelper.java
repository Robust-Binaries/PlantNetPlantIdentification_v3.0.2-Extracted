...
public class StackTraceHelper {
    ...
    public static class StackFrameImpl implements StackFrame {
        ...
        public JSONObject toJSON() {
            return new JSONObject(MapBuilder.m124of(UriUtil.LOCAL_FILE_SCHEME, getFile(), "methodName", getMethod(), StackTraceHelper.LINE_NUMBER_KEY, Integer.valueOf(getLine()), StackTraceHelper.COLUMN_KEY, Integer.valueOf(getColumn())));
        }
    }
    ...
    public static StackFrame[] convertJsStackTrace(@Nullable ReadableArray readableArray) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (type == ReadableType.Map) {
                ...
                StackFrameImpl stackFrameImpl = new StackFrameImpl(map.getString(UriUtil.LOCAL_FILE_SCHEME), map.getString("methodName"), (!map.hasKey(LINE_NUMBER_KEY) || map.isNull(LINE_NUMBER_KEY)) ? -1 : map.getInt(LINE_NUMBER_KEY), (!map.hasKey(COLUMN_KEY) || map.isNull(COLUMN_KEY)) ? -1 : map.getInt(COLUMN_KEY));
    ...
    public static StackFrame[] convertJsStackTrace(JSONArray jSONArray) {
        ...
        while (i < length) {
            try {
                ...
                StackFrameImpl stackFrameImpl = new StackFrameImpl(jSONObject.getString(UriUtil.LOCAL_FILE_SCHEME), jSONObject.getString("methodName"), (!jSONObject.has(LINE_NUMBER_KEY) || jSONObject.isNull(LINE_NUMBER_KEY)) ? -1 : jSONObject.getInt(LINE_NUMBER_KEY), (!jSONObject.has(COLUMN_KEY) || jSONObject.isNull(COLUMN_KEY)) ? -1 : jSONObject.getInt(COLUMN_KEY));
                stackFrameArr[i] = stackFrameImpl;
                i++;
