...
public class ProgressBarDrawable extends Drawable implements CloneableDrawable {
    ...
    public void draw(Canvas canvas) {
        if (!this.mHideWhenZero || this.mLevel != 0) {
            if (this.mIsVertical) {
                drawVerticalBar(canvas, IPTCConstants.IMAGE_RESOURCE_BLOCK_PRINT_FLAGS_INFO, this.mBackgroundColor);
                drawVerticalBar(canvas, this.mLevel, this.mColor);
            } else {
                drawHorizontalBar(canvas, IPTCConstants.IMAGE_RESOURCE_BLOCK_PRINT_FLAGS_INFO, this.mBackgroundColor);
    ...
    private void drawHorizontalBar(Canvas canvas, int i, int i2) {
        ...
        int width = ((bounds.width() - (this.mPadding * 2)) * i) / IPTCConstants.IMAGE_RESOURCE_BLOCK_PRINT_FLAGS_INFO;
        int i3 = bounds.left + this.mPadding;
        int i4 = bounds.bottom - this.mPadding;
        int i5 = this.mBarWidth;
        int i6 = i4 - i5;
        this.mRect.set((float) i3, (float) i6, (float) (i3 + width), (float) (i6 + i5));
        drawBar(canvas, i2);
    }

    private void drawVerticalBar(Canvas canvas, int i, int i2) {
        Rect bounds = getBounds();
        int height = ((bounds.height() - (this.mPadding * 2)) * i) / IPTCConstants.IMAGE_RESOURCE_BLOCK_PRINT_FLAGS_INFO;
        int i3 = bounds.left + this.mPadding;
