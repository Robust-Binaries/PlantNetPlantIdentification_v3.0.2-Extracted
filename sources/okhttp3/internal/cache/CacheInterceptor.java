...
import okhttp3.C1512Response.Builder;
...
public final class CacheInterceptor implements Interceptor {
    ...
    public C1512Response intercept(Chain chain) throws IOException {
        ...
        C1512Response response = internalCache != null ? internalCache.get(chain.request()) : null;
        CacheStrategy cacheStrategy = new Factory(System.currentTimeMillis(), chain.request(), response).get();
        Request request = cacheStrategy.networkRequest;
        C1512Response response2 = cacheStrategy.cacheResponse;
        ...
        try {
            C1512Response proceed = chain.proceed(request);
            if (proceed == null && response != null) {
            }
            if (response2 != null) {
                if (proceed.code() == 304) {
                    C1512Response build = response2.newBuilder().headers(combine(response2.headers(), proceed.headers())).sentRequestAtMillis(proceed.sentRequestAtMillis()).receivedResponseAtMillis(proceed.receivedResponseAtMillis()).cacheResponse(stripBody(response2)).networkResponse(stripBody(proceed)).build();
                    proceed.body().close();
            ...
            C1512Response build2 = proceed.newBuilder().cacheResponse(stripBody(response2)).networkResponse(stripBody(proceed)).build();
            if (this.cache != null) {
    ...
    private static C1512Response stripBody(C1512Response response) {
        return (response == null || response.body() == null) ? response : response.newBuilder().body(null).build();
    }

    private C1512Response cacheWritingResponse(final CacheRequest cacheRequest, C1512Response response) throws IOException {
