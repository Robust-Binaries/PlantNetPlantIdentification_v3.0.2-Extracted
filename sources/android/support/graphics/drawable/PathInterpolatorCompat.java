...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        if (min > 0) {
            this.f10mX = new float[min];
            this.f11mY = new float[min];
            float[] fArr = new float[2];
            for (int i2 = 0; i2 < min; i2++) {
                pathMeasure.getPosTan((((float) i2) * length) / ((float) (min - 1)), fArr, null);
                this.f10mX[i2] = fArr[0];
                this.f11mY[i2] = fArr[1];
            }
            if (((double) Math.abs(this.f10mX[0])) <= 1.0E-5d && ((double) Math.abs(this.f11mY[0])) <= 1.0E-5d) {
                int i3 = min - 1;
                if (((double) Math.abs(this.f10mX[i3] - 1.0f)) <= 1.0E-5d && ((double) Math.abs(this.f11mY[i3] - 1.0f)) <= 1.0E-5d) {
                    int i4 = 0;
                    float f = 0.0f;
                    while (i < min) {
                        float[] fArr2 = this.f10mX;
                        int i5 = i4 + 1;
                        float f2 = fArr2[i4];
                        if (f2 >= f) {
                            fArr2[i] = f2;
            ...
            sb2.append(this.f10mX[0]);
            sb2.append(",");
            sb2.append(this.f11mY[0]);
            sb2.append(" end:");
            int i6 = min - 1;
            sb2.append(this.f10mX[i6]);
            sb2.append(",");
            sb2.append(this.f11mY[i6]);
    ...
    public float getInterpolation(float f) {
        ...
        int i = 0;
        int length = this.f10mX.length - 1;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f10mX[i2]) {
                length = i2;
        ...
        float[] fArr = this.f10mX;
        float f2 = fArr[length] - fArr[i];
        if (f2 == 0.0f) {
            return this.f11mY[i];
        }
        float f3 = (f - fArr[i]) / f2;
        float[] fArr2 = this.f11mY;
        float f4 = fArr2[i];
