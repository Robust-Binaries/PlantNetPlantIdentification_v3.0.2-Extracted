...
class MathKt__MathJVMKt extends MathKt__MathHKt {
    ...
    public static final double asinh(double d) {
        if (d >= Constants.taylor_n_bound) {
            if (d <= Constants.upper_taylor_n_bound) {
                ...
                double d3 = (double) 1;
                Double.isNaN(d3);
                return Math.log(d + Math.sqrt(d2 + d3));
    ...
    @InlineOnly
    /* renamed from: ln */
    private static final double m187ln(double d) {
        return Math.log(d);
    }
    ...
    @InlineOnly
    /* renamed from: ln */
    private static final float m188ln(float f) {
